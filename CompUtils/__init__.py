...
def _calculate_lookup_attainment(ach, lookup_table):
	return sum(map(
		lambda x: (min(x['finish'], ach) - x['start']) * x['rate'] + x['base'] if ach > x['start'] and ach < x['finish'] else 0, lookup_table))


calculate_lookup_attainment = numpy.vectorize(_calculate_lookup_attainment)
